...
*   **Native web search, not OpenRouter's own.** `engine: "native"` uses the provider's built-in
    grounding (Google Search for Gemini) and OpenRouter normalises the citations into
    OpenAI-shaped `url_citation` annotations, so one parser covers every provider.
...
class OpenRouterProvider:
    ...
    def _body(self, spec: CallSpec) -> dict[str, Any]:
        ...
        if spec.web:
            body["plugins"] = [
                {"id": "web", "engine": "native", "max_results": spec.max_results}
            ]
        return body
        ...
        for attempt in range(1, self.max_attempts + 1):
            try:
                ...
                if response.status_code == 404:
                    raise ModelUnavailable(f"{spec.slug} is not available on OpenRouter")
